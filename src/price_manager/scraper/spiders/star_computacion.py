...
class StarComputacionSpider(scrapy.Spider):
    ...
    async def start(self):
        for prod_id, nombre in self.productos_internos:
            ...
            if not ruta:
                self.logger.warning(f"Sin categoria para '{nombre}', salteando.")
                continue
    ...
    def parse(self, response):
        ...
        for tarjeta in tarjetas[:10]:
            href = tarjeta.css('::attr(href)').get()
            datos = {
                'producto_id': prod_id,
                'nombre_buscado': nombre,
                'url_img_tarjeta': tarjeta.css('.img::attr(src)').get(''),
    ...
    def parse_detalle(self, response):
        ...
        url_img = response.css(
            "img[src*='files/products']:not([class])::attr(src)").get('')
        loader.add_value(
            'url_img',
